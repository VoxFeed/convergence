const {first, pick, filter, isEmpty, get, values, omit, has} = require('lodash');
...
const MemoryCrud = (engine, model) => {
  ...
  const upsert = (data, query = {}) => _upsert(snakeobj(data), query)
  ...
  const _insert = data => {
    ...
    return _validateRequiredFields(dataToSave, model)
      .then(() => {
        ...
        return _insertParent(data)
          .then(parent => Object.assign(response, parent))
          .then(() => _insertChild(data, response))
          .then(child => Object.assign(response, child));
      });
  };
  ...
  const _insertOnStore = (record, _model) => {
    ...
    return new Promise((resolve, reject) => {
      ...
      if (_primaryKeyExists(record, _model)) return reject(CantInsertRecordError());
      if (_uniqueIndexKeyExists(record, _model)) return reject(CantInsertRecordError());
