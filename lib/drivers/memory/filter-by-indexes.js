const OPERATORS = ['gt', 'gte', 'lt', 'lte', 'and', 'or'];
...
const FilterByIndexes = (model, store) => {
  ...
  const applyFilter = query => !hasOperators(query) ? filterWithoutOperators(query) : handleResponse(store[collection]);

  const hasOperators = (query) => {
    return OPERATORS.reduce((result, op) => {
      return result && has(query, op) && true;
    }, true);
  };

  const filterWithoutOperators = query => {
