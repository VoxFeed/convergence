...
const MemoryFilter = (schema, store) => {
  const filterQuery = (query) => {
    ...
    let response = filter(_getDataStore(), item => applyFilter(item, where));
  ...
  const _getDataStore = () => {
    if (!schema.isExtended()) return store[schema.collection];

    const extendedModel = schema.getExtendedModel();
    const dataStore = store[schema.collection];
    const dataExtendedStore = store[extendedModel.model.collection];

    return _mergeStores(dataStore, dataExtendedStore, extendedModel.foreignKey);
  };

  const _mergeStores = (store, extendedStore, fk) => {
    return store.map(storeItem => {
      const extendedStoreItem = filter(extendedStore, {id: storeItem[fk]})[0];
