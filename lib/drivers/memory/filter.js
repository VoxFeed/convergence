...
const MemoryFilter = (schema, store) => {
  ...
  const meetsNestedFilterCriteria = (fieldOrOp, item, val) => {
    const nestedFields = getNestedFields(fieldOrOp);
    let res = item;
    nestedFields.forEach(field => res = res[field]);
    return equals(res, val);
  };
