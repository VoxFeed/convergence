const {toPlainObject, pick, clone, camelCase} = require('lodash');
...
const MongoCrud = (engine, model) => {
  ...
  const _extendedFindOne = query => {
    ...
    return findOneOp(extended, _getExtendedCollection())
      .then(_findParent(parent))
      .then(_respondCamelized);
  };

  const _findParent = parentQuery => data => {
  ...
  const _extendedRemove = query => {
    const {parent, extended} = transpiler.select(query);
    return removeOp(parent, getCollection())
      .then(_removeExtended(extended))
      .then(_respondCamelized);
  };

  const _removeExtended = extendedQuery => data => {
    const foreignKeyValue = data[_getForeignKey()];
    const query = _setForeignKeyToQuery(clone(extendedQuery), foreignKeyValue);
    return removeOp(query, _getExtendedCollection())
