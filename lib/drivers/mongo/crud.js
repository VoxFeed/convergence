...
const MongoCrud = (engine, model) => {
  ...
  const _extendedFindOne = query => {
    const mongoQuery = transpiler.select(query);
    return findOneOp(mongoQuery, getCollection())
      .then(_extendWithChild)
      .then(_respondCamelized);
  };

  const _extendWithChild = data => {
    const foreignKey = data[camelCase(model.getExtendedModel().foreignKey)];
    const mongoQuery = transpiler.select({where: {id: foreignKey}});
    return findOneOp(mongoQuery, _getExtendedCollection())
      .then((childData) => {
