...
const MongoCrud = (engine, model) => {
  ...
  const count = query => {
    ...
    return buildMongoExecute(connection)
      .then(({count}) => count(mongoQuery, getCollection()));
  };

  const find = query => {
    const mongoQuery = transpiler.select(query);
    return buildMongoExecute(connection)
      .then(({find}) => find(mongoQuery, getCollection()))
  ...
  const _findOne = query => {
    const mongoQuery = transpiler.select(query);
    return buildMongoExecute(connection)
      .then(({findOne}) => findOne(mongoQuery, getCollection()))
      .then(_respondCamelized);
  };
  ...
  const _extendedFindOne = query => {
    ...
    return buildMongoExecute(connection)
      .then(({findOne}) => {
        return {
          findOne,
          data: findOne(mongoQuery, getCollection())
        };
      })
      .then(({findOne, data}) => {
  ...
  const _executeRemove = query => {
    ...
    return buildMongoExecute(connection)
           .then(({remove}) => remove(mongoQuery, getCollection()));
  };

  const _executeUpdate = (query, data) => {
    const mongoQuery = transpiler.update(query, data);
    return buildMongoExecute(connection)
       .then(({update}) => update(mongoQuery, getCollection()));
  };
