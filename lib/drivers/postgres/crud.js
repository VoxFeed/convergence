...
const PostgresCrud = (engine, schema) => {
  ...
  const upsert = data => {
    ...
    const query = Object.keys(sanitizedData).reduce((result, field) => {
      if (schema.getUniqueIndexes().includes(field)) {
        result[field] = sanitizedData[field];
      };
      return result;
    }, {});
    return _executeUpsert({where: query}, sanitizedData).then(_respondSingle);
  };
