const {first, flatten, omit, get} = require('lodash');
...
const ID_PLACEHOLDER = '{idPlaceHolder}';

const PostgresTranspiler = model => {
  const tableName = model.collection;
  const parentModel = get(model.getExtendedModel(), 'model', {});
  ...
  const _extendedInsert = data => {
    const {name, foreignKey} = model.getExtendedModel();
    return _buildParentInsertSQL(data) + _buildChildInsertSQL(data) + ';' +
      ` SELECT * FROM ${tableName} JOIN ${name} ON ${tableName}.${foreignKey}=${name}.id`;
  };
  ...
  const _buildChildInsertSQL = data => {
    ...
    childData[foreignKey] = ID_PLACEHOLDER;
    return _buildBasicInsertSQL(model, childData)
      .replace(ID_PLACEHOLDER, '(SELECT id FROM NEW_PARENT_RECORD)');
  };
  ...
  const _extendedSelect = query => {
    ...
    const parentContidions = _buildParentConditions(where);
    const childConditions = _buildChildConditions(where);

    return `SELECT * FROM ${tableName} JOIN ${parentTable} ` +
      `ON ${tableName}.${foreignKey}=${parentTable}.id WHERE` +
      (isEmpty(parentContidions) ? '' : ` ${parentContidions}`) +
      (isEmpty(childConditions) ? '' : ` AND ${childConditions}`);
  };
  ...
  const _buildParentConditions = query => {
    const parentQuery = _getParentData(query);
    return _buildConditions(parentQuery, parentModel);
  };
  ...
  const _buildChildConditions = query => {
    const childQuery = _getChildData(query);
    return _buildConditions(childQuery, model);
  };
  ...
  const upsert = (query, data) => {
    return _buildBasicInsertSQL(model, data) +
      _buildConflictCondition(query, data, model.getUniqueIndexes()) +
      ' RETURNING *';
  };
  ...
  const _buildQueryConditions = query => {
    const conds = query ? _buildConditions(query, model) : null;
  ...
  const _buildConditions = (query, _model) => {
    const fieldsOrOps = Object.keys(query);
    return fieldsOrOps.map(_buildCondition(query, _model)).join(AND);
  };

  const _buildCondition = (query, _model) => {
    return field => {
      if (isSubQuery(query[field])) return _generateSubQuery(field, query[field], _model);
      if (isDotNotation(field)) return _buildConditionForDotNotation(field, query);
      return _buildConditionForRegularField(field, query, _model);
    };
  };
  ...
  const _buildConditionForRegularField = (field, query, _model) => {
    const type = _model.getFieldType(field);
    const value = generateValueForCondition(type, query[field]);
    return `${_model.collection}.${field}=${value}`;
  };
  ...
  const buildConditionsWithOperator = (field, op, val, _model) => {
    const type = _model.getFieldType(field);
  ...
  const _isIdPlaceHolder = val => val === ID_PLACEHOLDER;
