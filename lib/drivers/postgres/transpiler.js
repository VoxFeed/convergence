...
const PostgresTranspiler = model => {
  ...
  const upsert = (data, query) => {
    ...
    const {where} = snakeobj(query || {});
    if (_hasSingleIndexes()) return _upsertWithSingleIndexes(sanitizedData, where);
    if (_hasCombinedIndexes()) return _upsertWithCombinedIndexes(sanitizedData, where);
    if (!primaryKey) return insert(sanitizedData);

    return _buildBasicInsertSQL(model, sanitizedData) +
      _buildConflictCondition(sanitizedData, where, [primaryKey]) + ' RETURNING *';
  };

  const _hasSingleIndexes = () => model.getUniqueSingleIndexes().length > 0;

  const _hasCombinedIndexes = () => model.getUniqueCombinedIndexes().length > 0;

  const _upsertWithSingleIndexes = (data, where) => {
    const indexes = model.getUniqueSingleIndexes();
    return _buildBasicInsertSQL(model, data) +
      _buildConflictCondition(data, where, indexes) + ' RETURNING *';
  };

  const _upsertWithCombinedIndexes = (data, where) => {
    const indexes = model.getUniqueCombinedIndexes();
    return _buildBasicInsertSQL(model, data) +
      _buildConflictCondition(data, where, indexes) + ' RETURNING *';
  };
  ...
  const _buildConflictCondition = (data, where, indexes = []) => {
    if (indexes.length) return _buildConflictWithIndexes(data, where, indexes);
    return _buildConflictWithIndexes(data, where, [primaryKey]);
  };

  const _buildConflictWithIndexes = (data, where, indexes) => {
    const conds = _buildQueryConditions(where);
    const values = _buildSetUpdateValues(omit(data, primaryKey));
