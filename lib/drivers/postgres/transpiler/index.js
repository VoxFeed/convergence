...
const PostgresTranspiler = model => {
  ...
  const {buildConditions, buildQueryConditions} = conditionBuilders;
  const {buildConditionsAndSorting} = conditionBuilders;
  ...
  const _buildExtendedUpdateSQL = (where, data) => {
    ...
    return `UPDATE ${table}` + buildSetUpdateValues(data) +
      ` FROM ${tableName}` + buildQueryConditions(where).replace(ID_REPLACER, `${tableName}.${foreignKey}`);
  };
