const {isEmpty, first, omit, reduce, isPlainObject, isArray} = require('lodash');
...
const PostgresTranspiler = model => {
  ...
  const _extendedUpdate = (query, data) => {
    ...
    const updatedQuery = _getUpdatedQuery(query, data);
    return _buildParentUpdateSQL(query, data) +
      _buildBasicUpdateSQL(childQuery, model.getData(data)).replace(ID_REPLACER, idQuery) +
      '; ' + extendedSelect({where: updatedQuery});
  };

  const _getUpdatedQuery = (query, data) => {
    return reduce(query, (res, value, key) => {
      if (isPlainObject(value)) res[key] = _getUpdatedQuery(value, data);
      else if (isArray(value)) res[key] = value.map(item => _getUpdatedQuery(item, data));
      else res[key] = data.hasOwnProperty(key) ? data[key] : value;
      return res;
    }, {});
  };
