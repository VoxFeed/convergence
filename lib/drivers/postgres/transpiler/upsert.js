...
const buildUpsertTranspiler = model => {
  ...
  const _upsertExtendedModel = (data, where) => {
    ...
    return _buildUpsertSQL(parentModel, data, where) +
      _buildUpsertSQL(model, data, where) + select({where});
  };
  ...
  const _upsertSingleModel = (data, where) => {
    if (_hasSingleIndexes(model)) return _upsertWithSingleIndexes(model, data, where) + ' RETURNING *';
    if (_hasCombinedIndexes(model)) return _upsertWithCombinedIndexes(model, data, where) + ' RETURNING *';
  ...
  const _upsertWithSingleIndexes = (_model, data, where) => {
    ...
    return buildBasicInsertSQL(_model, data) +
      _buildConflictCondition(_model, data, where, indexes);
  };
