...
const modelFunctions = [
  'getSchemaFields', 'getKnownFields', 'getFieldType', 'unique',
...
const defineModel = (args = {}) => {
  ...
  const getSchemaFields = () => Object.keys(definition);

  const getKnownFields = data => {
    return Object.keys(snakeobj(data)).filter(isKnownField);
  };

  const getOwnFields = data => {
  ...
  const insert = data => {
    const dataToSave = beforeSaveFn(data);
    return crud.insert(dataToSave);
  };

  const upsert = (data, query) => {
    const dataToSave = beforeSaveFn(data);
    return crud.upsert(dataToSave, query);
  };

  const update = (query, data) => {
    const dataToSave = beforeSaveFn(data);
    return crud.update(query, dataToSave);
  };

  const schema = {
    collection, getKnownFields, getFieldType, isExtended, isKnownField,
    unique, present, getRequiredFields, getUniqueSingleIndexes,
    getUniqueCombinedIndexes, extend, getExtendedModel, getOwnFields, beforeSave,
    isOwnField, getCollectionForField, setPrimaryKey, getPrimaryKey, getData, insert,
    upsert, update, getSchemaFields
  };
